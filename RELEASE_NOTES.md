...
- **NEW:** Added **per-profile selection for Chromium-based browser import**, mirroring the existing Firefox profile picker. Choose a profile with `--browser-profile "Profile 1"` (the tool lists the available profiles when several exist) or via **Detect Profiles** on the Web Dashboard. The cookie database is resolved directly, so both the legacy `<profile>/Cookies` and the newer `<profile>/Network/Cookies` layouts work, and `--cookie-file` can point at a specific Cookies database
