...
- **BUGFIX:** **Unconfigured webhooks are not reported as broken** - Leaving **`WEBHOOK_URL`** at its shipped `your_webhook_url` placeholder now counts as "not configured" instead of failing **`--doctor`** with `Webhook URL is not a complete HTTPS URL`. The placeholder is also no longer treated as a real destination elsewhere, so an enabled webhook that was never filled in reports missing setup once rather than logging a delivery error on every alert
