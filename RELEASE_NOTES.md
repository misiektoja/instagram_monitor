...
- **NEW:** Added **detailed follower logging** (`--detailed-followers` flag or `DETAILED_FOLLOWER_LOGGING` config option) - forces the tool to download and compare the full list of followers/followings even if the total count hasn't changed, allowing the detection of user handle changes or simultaneous additions and removals; also stores extended profile metadata (User ID, Full Name, Profile Pic URL) in history JSON files (thanks [@tomballgithub](https://github.com/tomballgithub) for the idea and @Sha-Dox](https://github.com/Sha-Dox) for initial implementation)
