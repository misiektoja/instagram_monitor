...
Version **4.0** reads follower lists over **less than half the Instagram requests** previous versions needed. It adds **follower-list sources**, a **daily identity budget** that is on by default and an **account circuit breaker** with recovery after session repair. Setup now asks what to collect before asking where to read it from. It also adds **private SMTP password entry**, clearer Doctor checks and quieter diagnostics, with stronger protection for saved history and credentials. Signed-in monitoring keeps working after Instagram retired the profile endpoint earlier versions depended on.

**Features and improvements**:

- **NEW:** **Follower lists over fewer requests** - `FOLLOW_LIST_SOURCE = "auto"` reads lists over REST, which returns 25 accounts per request instead of the 12 the GraphQL queries returned, so a list of 1,000 accounts costs 40 requests instead of 84. GraphQL is retried only for missing or unreadable endpoints. Select `graphql` or `rest` explicitly if needed. Lists require a logged-in session and challenges or rate limits never trigger fallback
- **NEW:** **Profile URLs as targets** - A target may now be a complete profile URL such as `https://www.instagram.com/someuser/`, in the setup wizard, on the command line and in `TARGET_USERNAMES`. It is stored as the username. A post or story URL is still refused, with the reason
- **NEW:** **Connection choices** - The `--setup` wizard asks whether to collect followers and following, followers only, or counts only with no names, and asks where to read them from only when it will read them. Counts only is the default, since names are the one thing Instagram acts against. The wizard and Web Dashboard can also choose the HTTP backend, curl_cffi browser impersonation and follower-list source
- **NEW:** **Experimental browser source** - `FOLLOW_LIST_SOURCE = "browser"` requires the `playwright` extra and an installed browser. It is slower and never selected automatically. Read the [browser-source limitations](https://misiektoja.github.io/instagram_monitor/usage/#browser-source-experimental). Thanks to [@tomballgithub](https://github.com/tomballgithub) for validation and feedback
- **NEW:** **Daily identity budget** - `IDENTITY_BUDGET_PER_DAY` limits follower and following name fetching across targets until local midnight. It defaults to **2000**, which covers one full follower and following scan of a typical account with room to repeat it. Counts and other activity continue when exhausted and incomplete lists never replace saved history. A scan needing more names than the budget still allows is skipped in full and says so, so raise the budget above the largest list you monitor. Monitors sharing an output directory share one ledger, and each change to it is made under a lock the operating system holds, so their daily counts cannot overwrite each other
- **NEW:** **Account circuit breaker** - Enabled by default, `CIRCUIT_BREAKER` stops all targets using an account after a confirmed challenge, checkpoint or expired session. Fix the account issue and restart or re-import the session to check recovery. No manual clearing is needed. Unreadable safety records keep monitoring stopped until repaired, and the error names the account whose record is at fault, since one bad record stops every account in the file
- **NEW:** **Exposure report** - `--exposure` shows identity usage, failure categories and circuit-breaker state without account names, target names or local paths. It also reports whether the ledger exists yet and whether this run could save it, how much of the daily budget is left and that it resets at local midnight, how many other accounts share the file and how many of them are stopped, and that every target using the account is paused while the breaker is tripped. A stop recorded before `CIRCUIT_BREAKER` was switched off is reported as kept rather than enforced. The ledger path is printed below the pasteable report, and the command exits with status `1` when the ledger cannot be read or cannot be saved
...
- **IMPROVE:** **Setup and Doctor checks** - Setup preserves saved settings, lets you skip unavailable answers and offers log, CSV and alert choices. Choosing an existing Instaloader session now checks that a session file for that account exists, instead of saving a login that fails on the first run. Turning a login on from the review menu asks which follower lists to collect, so enabling a session there cannot start name collection you were never asked about, and turning it off records counts only. Doctor names invalid settings, checks output paths and warns about polling below one hour. It also reports whether traffic goes through a **proxy** and where to, whether an enabled Web Dashboard or Terminal Dashboard can start on this machine, and whether the account safety ledger and the saved follower list directory are writable, which a run writes whatever the log and CSV settings say. It reports an account the **circuit breaker** has stopped and a saved session that signs in as a different account, both of which leave monitoring unable to run while every other check passes. Email settings the sender itself would refuse, such as a port that is not a number, are named without opening a connection. Before the checks start it states how many Instagram requests it will make. Repair commands remain usable when settings need correction
- **IMPROVE:** **Clearer browser session import** - The profile list in setup and on the Web Dashboard marks the profiles signed in to Instagram with `*`, and defaults to the signed-in one when there is only one. The setup login menu also reports which browsers have a signed-in profile at all, before you choose one. A number outside the list is re-asked instead of ending the command. A failed import in `--setup` offers another attempt or a different browser, so picking the wrong profile or the wrong browser no longer means finishing setup without a session. When no profile can be listed, the message says whether the browser is missing or simply has profiles that have never stored a cookie, instead of always blaming a missing install. A Firefox profile holding no Instagram cookies is now reported before any Instagram request, as the Chromium path already did, and both report which other profiles you could try. Importing from a browser that is still running now finds a session saved moments earlier, which previously read as no session at all in Firefox. On Linux, a `--browser-profile` name shared by the Snap, Flatpak and distribution builds of Firefox is refused with the full profile directories to choose from, instead of silently importing from whichever was found first, and Snap and Flatpak builds of Chromium and Brave are now found at all. For Chromium-based browsers `--browser-profile` also accepts the display name the picker shows, and a denied keychain or locked keyring is reported as such rather than as a missing login
- **IMPROVE:** **Fewer lookups after a restart, and the daily name cap is asked for** - A target's Instagram user id is now written beside the other state files and reused by later runs, so an established target is not looked up again on every start. An id is dropped and resolved again if Instagram reports a different name for it, which is what happens when a username is freed and taken by someone else. A target that is the signed-in account itself takes its id from the session and is never searched for (thanks [@VecToR-1024](https://github.com/VecToR-1024), from [#135](https://github.com/misiektoja/instagram_monitor/pull/135)). `--setup` also asks what **`IDENTITY_BUDGET_PER_DAY`** should be, which the question above it already named but no answer there could change. It is asked only when the setup collects names
- **IMPROVE:** **Reels counts are not recounted every cycle** - Instagram stopped answering the endpoint that reports a target's reels count, so the count is worked out by reading the whole reel list. That list is now read again only when the target's **posts count** moves, which a new reel does
- **IMPROVE:** **Quieter diagnostics and notifications** - `--verbose` reports events and `--debug` adds technical traces with secrets redacted. Persistent outages produce hourly reminders and recovery notices instead of repeated errors. Subjects omit program-name prefixes. Set `DELIVERY_CONFIRMATIONS = False` to hide delivery confirmations while keeping verbose diagnostics. The verbose startup summary names the **mail server**, the masked **recipient** and the **webhook provider** an alert would reach. A channel still holding the shipped sample values is reported as not configured instead of as a destination the run could use
- **IMPROVE:** **TLS and terminal output** - `VERIFY_SSL` covers outbound certificate checks, including email. Verification is on by default and disabling it produces a warning. `--truncate N` limits screen width while logs retain full lines. It works without `wcwidth`, which improves Unicode width measurements. Copy the updated `grc/conf.monitor_logs` to `~/.grc/` to use the live terminal colours in saved logs

**Bug fixes**:

- **BUGFIX:** **Reported changes name the window they were observed in** - The **`Check interval:`** line under a change was built from the randomized interval the run was about to sleep, not from the time that had passed, so a change found after a failed check or outside the monitoring hours was reported over a window the tool had not been watching. It now measures from the previous successful check
...
- **BUGFIX:** **Signed-in monitoring works again after Instagram retired a profile endpoint** - In September 2026 Instagram stopped answering `api/v1/users/web_profile_info/` for accounts that are signed in, replying `400 feedback_required` however healthy the account was. Every signed-in run failed at the first profile lookup, and 4.0 read that reply as an account-level block and stopped the account. A signed-in run now resolves the target through Instagram's **search** and reads the profile over **GraphQL**, which the same session answers normally. Each target's id is resolved once and reused for the rest of the run. The retired endpoint is still tried for a target search cannot list, and its refusal is now reported as `endpoint_retired`, which does not stop the account or trip the **circuit breaker**. Runs without a login were never affected and are unchanged (thanks [@VecToR-1024](https://github.com/VecToR-1024) for an independent fix and validation in [#135](https://github.com/misiektoja/instagram_monitor/pull/135))
- **BUGFIX:** **Windows Docker Firefox import finds the profiles** - The Windows container commands mounted `%APPDATA%\Mozilla\Firefox`, one level above the `Profiles` folder the container looks in, so the import reported no `cookies.sqlite` was found. The commands now mount `%APPDATA%\Mozilla\Firefox\Profiles`, and a container given the old path still finds the profiles, so a command already saved in a script keeps working
- **BUGFIX:** **A broken proxy no longer blocks the commands that correct it** - `PROXY_ENABLED` with an unusable `PROXY_URL` or a missing `PROXY_CERT_PATH` stopped every command, including **`--doctor`**, `--setup`, `--exposure`, `--clear-breaker`, `--analyze-follows` and the secret commands, so the tools that explain and repair the setting could not run. They now report the problem and continue with the proxy switched off. A monitoring run still stops. The message describes what is wrong with the value, since `PROXY_URL` is private and the old message printed only the redaction that replaced it
- **BUGFIX:** **Setup refuses answers the next run would reject** - `--setup` accepted any text as a **target**, as the **session account name** and as the **sender and receiver addresses**, then reported a finished setup. Monitoring stopped on the first run with `Instagram username must be 1-30 letters...`, and email alerts were refused at the first send even though setup had reported a successful mail server sign-in, which only proves the username and password. Each answer is now checked where you enter it, with the reason and a retry. The **SMTP host** is checked the same way
- **BUGFIX:** **Setup prints commands and addresses that match what it saved** - The browser import command shown after setup omitted `--config-file`, so it could not print its own next steps when setup had written the configuration somewhere other than the default. The Web Dashboard address was always printed as `http://127.0.0.1:8000/`, even with `WEB_DASHBOARD_PORT` set to something else. Starting the tool as `python -m instagram_monitor` also printed commands naming a downloaded script
- **BUGFIX:** **A saved SMTP password is not retyped only to be discarded** - Setup asked for the password at a hidden prompt and only afterwards asked whether to replace the saved one, so declining threw away what you had just typed. It now asks before the prompt, the same way the Instagram password does. An address containing spaces is also no longer accepted as a sender or receiver
- **BUGFIX:** **Session recovery no longer sends everyone to Firefox** - The `To fix:` line for an expired session, a challenge and a stopped account printed the Firefox import command and told the reader to log in through Firefox, whatever browser their session came from. Those messages now name **Chrome, Brave and Chromium** as well, so the command can be used as printed or adjusted in one place
- **BUGFIX:** **Flagged sessions caught across requests** - Challenges and feedback requirements during story, post or follower-list requests now pause other targets and trigger account recovery handling, as profile failures do. Fixes [#131](https://github.com/misiektoja/instagram_monitor/issues/131)
- **BUGFIX:** **Protected follower history** - Malformed follower and following records are reported before replacement. Correct the named file or move it aside to start fresh
- **BUGFIX:** **Safer configuration and secret updates** - Configuration replacement creates a backup when a filename is passed. Shell redirection bypasses this protection. Setup preserves retained credentials when changing destinations and keeps inline secrets out of configuration backups. A setup that collects no secret no longer writes an empty **`.env`** beside the configuration, and no longer records a path to it, so later runs do not open with a warning about a missing file. Exported secrets work without a dotenv file. Command-line credentials and nonempty startup exports retain priority after `SIGHUP`. Change those values and restart to replace them. Reloads apply changed or removed file-owned secrets
...
- **BUGFIX:** **Readable ntfy alerts** - ntfy notifications arrive as plain text. The bold markers, the backslashes in follower and following names and the brackets around links are gone, since no ntfy client renders the Discord markdown they came from. Discord alerts are unchanged
- **BUGFIX:** **Reliable startup and terminal output** - Invalid settings and unreadable dotenv files include repair guidance. Corrected colour handling avoids false error highlighting and redirected output avoids terminal-clearing errors. Windows requirements now include `colorama`
