...
Version **4.0** reads **follower and following lists from the web endpoints Instagram's own app calls**, with the older queries kept as a fallback and an **experimental browser source** for when both are broken, lets **setup and the Web Dashboard choose the HTTP backend and the list source**, adds a **daily identity budget** and an **account circuit breaker** that stops monitoring after Instagram challenges your account, a **TLS certificate verification switch**, gives **every rejected setup answer a way out**, **checks the mail server during setup**, adds **`--set-smtp-password`**, makes **debug traces scannable**, quiets **verbose output to events only**, adds the **install method and secret origins** to the complete startup summary, makes an **exported secret win over the dotenv file**, stops **`--generate-config` from replacing an existing config** without a backup, keeps **configuration upgrade notes visible** when monitoring moves into the full-screen Terminal Dashboard and brings Doctor, setup, the startup summary and the first screen in line with the sibling monitors.

**Features and improvements**:

- **NEW:** **Follower and following lists read from Instagram's current endpoints** - The lists now come from the REST endpoints Instagram's own web app calls, rather than the older GraphQL queries that stop working whenever Instagram retires one. **`FOLLOW_LIST_SOURCE`** picks the surface: `auto`, the default, reads over REST and retries over GraphQL only when the REST endpoint is missing or answers in a shape the tool cannot read, and only when nothing had been returned yet. `rest` and `graphql` pin one surface. A fetch that already returned names is never repeated on the other surface, and a rate limit, a challenge or an expired session is reported instead of retried, so a failed fetch never turns into extra requests against the account. The startup summary names the source in use and **`--follow-list-source`** pins it for one run. Anonymous mode is unchanged, since neither surface lists followers without a session
- **NEW:** **Setup and the Web Dashboard choose how requests reach Instagram** - **`--setup`** gains an **Instagram connection** section asking for the **HTTP backend**, the browser **curl_cffi** impersonates and, in login mode, the **follower list source**. It offers only the impersonation targets the installed curl_cffi accepts and defaults to `requests` when curl_cffi is missing. The same three settings are now editable in the Web Dashboard under **Settings** in the new **Instagram Connection** card, and appear in **Detailed Configuration**. Previously all three could only be set by editing the configuration file or passing a flag
- **NEW:** **Experimental browser follower list source** - When both API surfaces are broken, **`FOLLOW_LIST_SOURCE = "browser"`** reads the lists out of a real browser instead, opening the follower or following dialog on Instagram's own pages and scrolling it the way a person would. It needs the optional **`playwright`** extra and a downloaded browser, is much slower than either API surface and uses far more CPU and memory. **Instagram's terms forbid automated collection however it is done, and this can cost you the logged-in account**, so it is never selected by `auto` and there is no fallback to or from it. The browser reuses one profile per session account, is handed the existing session cookies rather than your password and uses your configured proxy. A challenge, a suspended or disabled account or a signed-out page stops the fetch and is reported rather than clicked through, and a dialog that stops growing short of the reported follower count fails instead of saving a short list over a complete baseline. Settings: **`FOLLOW_LIST_BROWSER_CHANNEL`**, **`FOLLOW_LIST_BROWSER_HEADLESS`**, **`FOLLOW_LIST_BROWSER_PROFILE_DIR`**, **`FOLLOW_LIST_BROWSER_SCROLL_DELAY`** and **`FOLLOW_LIST_BROWSER_TIMEOUT`**. **`--doctor`** reports whether Playwright and the browser are actually installed
- **NEW:** **Daily identity budget for follower and following names** - **`IDENTITY_BUDGET_PER_DAY`** caps how many follower and following names are fetched per day for the logged-in account, shared by every monitored target. Identity scans run one at a time so concurrent workers cannot spend the same allowance. REST pages are counted when Instagram returns them, including names the caller does not consume, so the ledger records actual exposure before another request is allowed. Instagram weighs automated collection by how much user-identifiable information a response returns rather than by request count, so names are the unit that matters and batch sizes matter less than the daily total. Once the budget is spent, name fetching stops until local midnight while counts, posts, reels, stories and profile changes keep being monitored. A budget-truncated list is never saved over the last complete baseline, so a stopped fetch cannot report unfetched accounts as unfollows. The budget is off by default and names are counted either way, so you can measure your usage with the new **`--exposure`** command before choosing a value. Also settable per run with **`--identity-budget`**
- **NEW:** **Circuit breaker stops the account after Instagram challenges it** - When Instagram returns a challenge, a checkpoint or an expired session it has acted against the account rather than against one request and continuing to send requests is what turns a warning into a suspension. **`CIRCUIT_BREAKER`** is on by default and now stops every monitored target at once on the first such response. The stop is held in memory before the ledger is written and stored state is checked before a client or session is created. It stays stopped across restarts until you clear the challenge in a browser and run **`--clear-breaker`**. An unreadable or unwritable safety ledger also stops authenticated monitoring instead of losing the breaker or identity total. Rate limits, ordinary HTTP 400 responses, network errors and Instagram API changes do not trip it. Previously a flagged session paused other targets only while a dashboard was running and resumed on the next start
- **NEW:** **`--exposure` prints a redacted support report** - Prints the version, runtime context, HTTP backend, follower-list source, session mode, identity total, sanitized failure counts and circuit-breaker state. It omits account names, target names, stored error text and local paths so the output can be pasted into a support issue or discussion #128. The ledger remains a local file next to the output directory and is never transmitted anywhere
- **IMPROVE:** **Readable debug traces and clearer verbose output** - Every **`--debug`** line now names the operation, then lists its details as comma-separated `key=value` fields, so a long trace is scannable instead of one run-on sentence. Every outbound call reports `outcome=OK` or `outcome=failed` with an `error=` field. **`--verbose`** now says when an alert channel was switched off because `SMTP_HOST` or `WEBHOOK_URL` is still a placeholder. A `--debug` run also leaves the terminal as it was instead of clearing it, so the output you are comparing against stays on screen, while `--verbose` clears it like an ordinary run. Both match the sibling monitors
- **IMPROVE:** **Verbose no longer prints a block per check** - `--verbose` used to print a starting line, a completed line and the check schedule on every cycle, which buried the events worth reading behind hundreds of identical blocks. Those now appear only under `--debug`. Verbose keeps the startup summary, follower and following counts and the other operational notices. The liveness banner prints in any mode. The notice that says a check was skipped outside **`CHECK_POSTS_IN_HOURS_RANGE`** now closes with the same timestamp block as every other verbose notice.
- **NEW:** **One switch for TLS certificate verification** - **`VERIFY_SSL`** controls certificate checking for every outbound connection: Instagram, the connectivity check, the proxy IP lookup, downloaded media, the mail server that sends email alerts and webhook deliveries. It stays `True` by default. Set it to `False` only on a network that intercepts TLS with its own certificate authority, which also leaves `PROXY_CERT_PATH` unused since there is nothing left to check the certificate against. The startup summary shows the state and moves that row into the concise view while verification is off, **`--doctor`** reports a warning naming the setting, and the repeated certificate warning is silenced so the choice is reported once instead of on every request
- **IMPROVE:** **Email setup is checked before it is saved** - **`--setup`** now signs in to the mail server after the last email question, so a wrong password or an unreachable host is reported during setup instead of at the first alert. No email is sent. A refused sign-in offers the mail server questions again, and giving up switches email alerts off. An unreachable server keeps the answers instead, so `--doctor` can check them once the machine is back online. The mail server questions now show the values already saved as defaults, ask for the password last where a blank answer keeps the stored one, and use the same wording and order as the sibling monitors. The SMTP port question now refuses a number outside 1 through 65535, so setup can no longer save a port `--doctor` would then reject. Declining the retry offer at a number question keeps the value already saved instead of asking the same question again. The hints under a question now print plain rather than as warnings, the way the sibling monitors print them. Email and webhook alerts are now chosen from a short menu - status and errors, every supported event, or a custom choice per type - instead of a single fixed selection
- **NEW:** **`--set-smtp-password` saves the mail server password privately** - Enter `SMTP_PASSWORD` through a hidden prompt instead of editing the dotenv file by hand. The mail server has to accept the password before it is written and no email is sent, so a wrong password, or an app password the provider requires, is reported straight away. A refused sign-in leaves the dotenv file unchanged, and a password already saved there is replaced only after you confirm. The command ends with the same labelled next-step commands `--set-webhook-url` prints.
...
- **IMPROVE:** **Names, IDs and links have their own colours** - Usernames are `bright_cyan underline`, the numeric account ID is `bright_magenta` through the new **`id`** theme key and URLs are `blue underline`, so a name is never mistaken for a link. The `Targets:` row now colours every account it lists, not just the first, and the `User ID:` row is coloured at all. The same three colours mean the same three things in every monitor in this family
...
- **IMPROVE:** **Doctor checks the timing and count settings** - A setting such as `INSTA_CHECK_INTERVAL`, a follower batch limit, a check-hour range or `SMTP_PORT` holding a negative, out-of-range or non-numeric value passed every check and only broke during the run. `--doctor` now reports **`One or more numeric settings are invalid`** as a failure and names every setting that is wrong in that one row, so a whole configuration can be corrected in one pass
- **IMPROVE:** **Doctor fails a webhook that cannot deliver** - An enabled webhook whose `WEBHOOK_URL` was still the placeholder was reported as a warning, so `--doctor` exited `0` for a channel that could never send anything. That row now reports **`FAIL`**, matching every other invalid webhook setting and the sibling monitors, so a scripted preflight catches a broken channel. A webhook that is enabled with no alert types selected stays a warning
- **IMPROVE:** **Doctor names every output destination** - The Configuration section now checks the CSV file each target would write alongside its log file, and says when either is disabled instead of leaving the row out. Doctor also ends with the command that starts monitoring, carrying the same configuration and dotenv files it just checked, labelled so a failing report asks for the failures first
...
- **IMPROVE:** **The Instagram password replace question comes first** - When `.env` already holds `SESSION_PASSWORD`, **`--setup`** now asks whether to replace it before the hidden prompt rather than after it, so a saved password is never retyped only to be discarded. The sibling monitors already ask before collecting their main credential
- **IMPROVE:** **Doctor, setup and the first screen read the same as the sibling monitors** - Doctor's email row now reports the **SMTP sign-in** and each ready notification row lists the **alert categories** that channel would deliver. Failed session and email rows now **name the cause** instead of printing the raw exception, and keep the technical text on the detail line. The setup summary aligns its rows, names the **webhook provider**, lists the selected email and webhook alert categories and colour-codes values such as the targets, the polling interval and each enabled or disabled channel. Doctor now also checks **`LOCAL_TIMEZONE`**, resolving `Auto` through `tzlocal` and reporting an unusable zone as a failure instead of stopping `--doctor` before the report is printed. Every `To fix:` action now reads as one capitalised instruction without a trailing period, in Doctor and during monitoring alike. The no-argument screen explains the target format and offers a `--doctor` check. `--set-webhook-url`, the setup next steps and the delivery-test commands all use the same labelled command block. Doctor also reports the optional `colorama` package, and reports it only on Windows, since the classic Command Prompt is the only place it changes anything. **Hidden prompts look like every other question**, such as the Instagram or SMTP password prompt. Debug output stays off while a secret is typed. The Python row names the minimum version the tool supports. Detail lines now carry only what the label does not, so a disabled CSV file, log file or webhook channel no longer repeats itself on a second line. The time zone row names its value as **`Time zone: <zone>`** and the no-login session row states only what needs a login. Each row is now one block: the `To fix:` and `Guide:` lines sit indented under their marker alongside the detail. A link in a row's detail is shown in the link colour. Doctor now uses the four shared markers only, so the neutral `[ -- ]` row is gone. **`--analyze-follows`** now states its counts and snapshot times as plain value rows instead of borrowing a Doctor marker. Setup groups its questions the way the siblings group theirs, so the polling question opens its own block instead of running on from the target answer. It also stops creating an empty dotenv file when a run stores no secret. The commands it prints afterwards no longer name that file. Every command it prints for you to run is now the short portable form (`python3 instagram_monitor.py ...` for a downloaded script) and carries the `--config-file` or `--env-file` this run was given, so it can be pasted as it is, including the `none` value that switches either search off. **Starting with no target now prints the shared `* Error:`, `To fix:` and `Guide:` block** instead of the whole help screen, and a time zone that cannot be resolved reports itself the same way instead of three loose hint lines. Doctor's Connectivity section now opens with the shared endpoint row, naming the endpoint the tool contacts at startup, ahead of the service check The optional delivery tests now name the webhook provider in every result row and each row explains its outcome, so an approved test that failed points at the error above and a declined one says how to approve it next time. Doctor's progress line now uses the shared `* Checking <step> ...` shape instead of its own sentences, and a `--setup` run with no terminal points at the same command and files the siblings name. A target Instagram cannot fetch is now a failure, and an email or webhook channel that is configured with no alert types selected is reported as a warning. After saving, setup offers to start monitoring only after a doctor run passed. The review menu uses the sibling section labels, a `--setup` run with no terminal also prints the setup guide link and a switched-off destination such as `--config-file none` is refused with the shared message.
- **IMPROVE:** **The startup summary reads the same as the sibling monitors** - Values now line up in the same column as in the other monitors instead of being tab-aligned, so a long label no longer shifts the column. The rows appear in the shared order, ending with the environment block, and several use the shared wording: `Config`, `Dotenv`, `Polling interval`, `Session mode`, `Liveness output`, `CSV output`, `Output logging`, `Profile picture changes`, `Profile picture display` and `Web dashboard`. The concise view gains an `Output` row and ends with `More details: use --verbose or --debug` in place of the old `(run with --verbose to see all settings)` hint. A new `Coloured output` row reports whether colour is actually in use, and the secret origins are now three rows naming what came from the dotenv file, the environment and the configuration file
- **IMPROVE:** **Install method and effective secret origins in diagnostics** - `--verbose` and `--debug` show a readable install method and name the effective source of each secret. Doctor opens with the raw `manual`, `pip`, `docker` or `compose` method used by support reports. If a key exists in several places, diagnostics identify the source that supplied the value in use. Explicit command-line values are identified separately. Only secret names are shown, never values. The default concise startup view is unchanged
- **IMPROVE:** **A clearer `--help` screen and screen line truncation** - The argument groups now appear in the order shared with the sibling monitors, with **`Email notifications`** and **`Webhook notifications`** named apart so one group name no longer stands for both. `--setup`, `--doctor`, `--generate-config` and `--set-webhook-url` sit together under **`Configuration & dotenv files`**. The examples are grouped by task with a comment above each command saying what it is for. Every command is written for the detected install method. The one-shot commands - `--setup`, `--doctor`, the `--set-*` commands and the `--send-test-*` commands - describe themselves with the same sentence in every monitor, so the same command no longer reads as a different one in each tool. The new **`--truncate N`** flag and **`TRUNCATE_CHARS`** setting cut each screen line to a maximum width, with `999` to auto-detect the terminal width. Log files always keep the full line and truncation is off by default. It needs the optional `wcwidth` package to measure display width
- **IMPROVE:** **`--config-file none` switches config discovery off** - The value `none` already switched the dotenv search off through `--env-file none`. It now does the same for the configuration file, so a run can use built-in defaults and command-line values only, without picking up a `instagram_monitor.conf` sitting in the working directory. `--doctor` reports it as `[PASS] No configuration file selected` instead of `[FAIL] Error: Config file 'none' does not exist`, whose `Error:` prefix came from a startup message being reused as the check label. That prefix is now stripped from every Doctor label. `--setup` refuses both values, since it needs somewhere to write the files it saves. Both flags name the value in `--help`
- **IMPROVE:** **Test commands send the same message in every monitor** - **`--send-test-email`** and **`--send-test-webhook`** now use one subject and one body shared by the sibling monitors. The body names the command that sent it, so a test message arriving beside real alerts is easy to place The web dashboard's test buttons say the message came from the dashboard. **`--send-test-webhook`** with no destination now names how to set one and links the guide, rather than printing a bare error line
- **IMPROVE:** **Doctor warns about a polling interval below the documented minimum** - An **`INSTA_CHECK_INTERVAL`** below 3600 seconds now gets its own `[WARN]` row naming the interval in use and the setting to raise. A challenged account looks like a broken tool rather than a setting, so the warning arrives before the run instead of during it. The same row appears in every monitor in this family

**Bug fixes**:

- **BUGFIX:** **The liveness banner now follows the clock** - Its cadence was counted in checks derived from **`INSTA_CHECK_INTERVAL`**, so with a check interval longer than **`LIVENESS_CHECK_INTERVAL`** the banner printed after every check instead of once per interval. The banner and the outage reminder now print once per **`LIVENESS_CHECK_INTERVAL`** of elapsed time. The banner also names what was checked rather than printing a bare timestamp, whether or not **`--verbose`** is on
- **BUGFIX:** **A lasting failure no longer repeats itself every check** - A failure that kept recurring printed its error line and its `To fix:` paragraph on every check, which buried everything else in a long outage. Both are now printed once, then the liveness banner takes over with `* Monitoring degraded for <instagram_user>` and what is still failing, once per **`LIVENESS_CHECK_INTERVAL`** however often the failing run retries. Once it clears, `* Monitoring recovered for <instagram_user>` reports how long the outage lasted. With **`LIVENESS_CHECK_INTERVAL`** set to 0 there is no banner to carry the reminder, so the error line keeps printing on every check The report line itself now reads `* Error: <what failed> (retrying in <time>)`, the same shape every monitor in this family prints. The hint that the saved session may be at fault is now a `To fix:` line that gives way to a more specific fix rather than a second starred line repeating it
- **BUGFIX:** **A target answer with no usable name no longer loops in silence** - Setup asked which accounts to monitor and, given an answer that held no name such as a lone comma, asked again with nothing printed and no way out except Ctrl+C. It now says what a valid answer looks like then offers to continue without a target, saying where the accounts can be added later
...
- **BUGFIX:** **The printed monitoring command carries a usable target** - After `--doctor` and `--import-session`, the command printed under `start monitoring` left the target out whenever the run was given none, so pasting it started nothing. It now carries the targets the run used, leaves them out when the configuration file already supplies them and otherwise shows `<target_insta_user>` for you to replace, unless the Web Dashboard is enabled and targets can be added there
...
- **BUGFIX:** **Quoted text is coloured whole** - Quoted output such as a caption or a name was coloured only up to its first apostrophe, and any quoted value containing a dot, an underscore or a slash was left plain entirely, so `'Don't miss this'` and `'S.T.A.L.K.E.R. 2'` both came out wrong. Both are now coloured in full, while quoted file names and paths still stay plain. A quoted **`<placeholder>`**, a quoted command-line option such as `'--env-file none'` and a quoted piece of a URL such as `'?code='` are no longer coloured as content
- **BUGFIX:** **The `email` colour is used again** - `COLOR_THEME` listed an `email` colour, but every line this tool prints about sending one starts with `* Sending email`, and those lines were returned plain before the colour was applied, so the setting did nothing. A delivery announcement now carries it, the way the matching `webhook` line already did
- **BUGFIX:** **The `signal` colour is used again** - `COLOR_THEME` listed a `signal` colour that nothing ever read, so setting it did nothing. A received signal line such as `* Signal SIGUSR1 received` now uses it, which tells a control signal apart from a warning at a glance
...
- **BUGFIX:** **A saved secret is not replaced without asking** - Typing an Instagram or mail server password during **`--setup`** overwrote the value already in `.env` without a word. Setup now asks first and keeps the stored value when you decline, which the webhook URL and the ntfy access token already did
...
- **BUGFIX:** **Exported secrets take precedence over the dotenv file** - A secret exported in the environment, such as `SESSION_PASSWORD`, `SMTP_PASSWORD`, `WEBHOOK_URL`, `PROXY_URL` or `NTFY_ACCESS_TOKEN`, now wins over the same key in the dotenv file, matching how `python-dotenv` and container or service-manager setups already behave. Previously the file replaced the exported value, so a one-off run or a secret injected by systemd or Docker was silently ignored. Version 3.9.1 made these variables work without a dotenv file and they now also win when both are present. Reloading with **`SIGHUP`** still applies the edited dotenv file, so rotating a secret there keeps working
- **BUGFIX:** **`--generate-config` no longer replaces an existing config without a backup** - Writing the template to a file that already exists now keeps the previous version in a timestamped **`.bak`** file beside it and prints where it went. Previously the file was overwritten in place, which could discard a tuned configuration. Shell redirection with `>` is handled by the shell and still overwrites without asking
- **BUGFIX:** **No doubled `To fix:` prefix** - Running **`--doctor`** with a `--config-file` path that does not exist reported the action as `To fix: To fix: ...`. The report now prints the prefix once
...
- **BUGFIX:** **A cancelled secret command explains what to do next** - Ctrl+C or an `n` at the `Replace the saved ...?` question of `--set-webhook-url` or `--set-smtp-password` printed a single error line with no `To fix:` and no guide link. A decline was also reported as a cancellation. Both outcomes now print the same block as any other failure. A cancel names the command that resumes it while a declined replacement says the saved value was left as it is
- **BUGFIX:** **Unusable email settings are one warning that names them** - `--doctor` reported `[WARN] Email alerts are on but SMTP is not configured` with a detail that only repeated that nothing had been contacted. A placeholder `SENDER_EMAIL` or `RECEIVER_EMAIL` was a separate `[FAIL]`, which made it exit 1. Both are now `[WARN] Email alerts are enabled but unusable`, whose detail names the settings that are actually unset and whose `To fix:` names the same ones
...
- **IMPROVE:** **Clearer issue reports and contribution guide** - New issue forms ask for the version, install method, operating system, login method and `--doctor` output. Security problems are directed to private reporting. New [CONTRIBUTING.md](https://github.com/misiektoja/instagram_monitor/blob/main/CONTRIBUTING.md), [CODE_OF_CONDUCT.md](https://github.com/misiektoja/instagram_monitor/blob/main/CODE_OF_CONDUCT.md) and a pull request template explain how to contribute
- **IMPROVE:** **Published security policy** - [SECURITY.md](https://github.com/misiektoja/instagram_monitor/blob/main/SECURITY.md) explains how to report a security problem privately through **GitHub security advisories**. It also explains which versions receive fixes and safe deployment basics: keep the password-free Web Dashboard on your own machine, store secrets in owner-only **`.env`** files and use configuration files only for settings
- **IMPROVE:** **Clearer dependency and security checks** - New [THIRD_PARTY_NOTICES.md](https://github.com/misiektoja/instagram_monitor/blob/main/THIRD_PARTY_NOTICES.md) lists each dependency, its license and why the project uses it. Automated checks look for known vulnerable dependencies, create a list of installed components, scan the container image and scan the source code. They run on every change and weekly, so newly reported problems can still be found

**Bug fixes**:

- **BUGFIX:** **Reliable follower and following alerts** - The tool now ignores a changed follower count only after a complete list comparison confirms that no usernames changed. No-login mode, skipped or failed downloads and configured download limits still alert about count changes. An incomplete download is no longer saved as the new comparison list. The tool now also logs **Followers list changed** when one person follows and another unfollows between checks ([@stdpankaj](https://github.com/stdpankaj), closes [#118](https://github.com/misiektoja/instagram_monitor/issues/118) and [#119](https://github.com/misiektoja/instagram_monitor/issues/119))
