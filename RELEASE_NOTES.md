...
Version **4.0** adds new **follower-list sources** (auto/graphql/rest/browser), an optional **daily identity budget** and a **circuit breaker** that stops monitoring after an account challenge or expired session. Setup and the Web Dashboard can choose the connection backend and list source. It also adds a **`--set-smtp-password`** command for hidden password entry, clearer setup checks and quieter diagnostics, fixes secret precedence and protects configuration files during upgrades.

**Features and improvements**:

- **NEW:** **Follower-list sources** - defines which Instagram surface follower and following lists are read from; `FOLLOW_LIST_SOURCE = "auto"` now uses REST with a limited GraphQL fallback for missing or unreadable endpoints. You can select either source explicitly (`graphql` or `rest`). Lists still require a logged-in session and challenges or rate limits do not trigger fallback
- **NEW:** **Connection choices in setup and the Web Dashboard** - Choose the HTTP backend, curl_cffi browser impersonation and follower-list source without editing the configuration file
- **NEW:** **Experimental browser list source** - `FOLLOW_LIST_SOURCE = "browser"` reads follower lists through a real browser. It requires the `playwright` extra and an installed browser, runs more slowly and is never selected automatically. Read the [browser-source limitations](https://misiektoja.github.io/instagram_monitor/usage/#browser-source-experimental) before enabling it; thanks [@tomballgithub](https://github.com/tomballgithub) for validation of the idea and feedback
- **NEW:** **Daily identity budget** - `IDENTITY_BUDGET_PER_DAY` limits follower and following name fetching across targets until local midnight. It is disabled by default. Counts and other activity continue when it is exhausted and incomplete lists never replace saved baselines
- **NEW:** **Account circuit breaker** - Enabled by default, `CIRCUIT_BREAKER` stops all targets after a challenge, checkpoint or expired session and stays stopped across restarts. Resolve the account issue then run `--clear-breaker`. Unreadable or unwritable safety records also stop authenticated monitoring
- **NEW:** **Exposure report** - `--exposure` shows identity usage, failure categories and circuit-breaker state without account names, target names or local paths. Use it to choose a budget or troubleshoot list failures
- **NEW:** **Private SMTP password setup** - `--set-smtp-password` checks a hidden password with the mail server before saving. Guided setup also checks email credentials without sending a message
- **IMPROVE:** **Setup and Doctor checks** - Setup lets you skip unavailable answers, choose alert categories and configure log and CSV output. Doctor checks timing values, time zones and output paths, warns about polling below one hour and fails unusable webhook settings
- **IMPROVE:** **Quieter diagnostics** - `--verbose` focuses on events while `--debug` records individual checks. Persistent failures use periodic reminders and recovery notices. Detailed startup output identifies the install method and secret sources without revealing values
- **IMPROVE:** **TLS verification and terminal output** - `VERIFY_SSL` covers outbound connections, including email. It warns when disabled. Optional `--truncate N` limits screen width while logs retain full lines and requires `wcwidth`
...
- **BUGFIX:** **Reliable prompts and printed commands** - Invalid target answers no longer loop silently. Ctrl+C ends optional delivery tests and reports whether setup was saved. Printed monitoring commands include usable targets
- **BUGFIX:** **Correct colours and captured output** - Quoted names and captions remain fully coloured, email and signal theme settings work and `--no-color` covers Python 3.14 help. Redirected output no longer triggers terminal-clearing errors. Installing from `requirements.txt` on Windows now pulls `colorama`, so the classic Command Prompt gets the colours the packaged install already had
