...
- **NEW:** **Daily identity budget** - `IDENTITY_BUDGET_PER_DAY` limits follower and following name fetching across targets until local midnight. It defaults to **2000**, which covers one full follower and following scan of a typical account with room to repeat it. Counts and other activity continue when exhausted and incomplete lists never replace saved history.
- **NEW:** **Account circuit breaker** - Enabled by default, `CIRCUIT_BREAKER` stops all targets using an account after a confirmed challenge, checkpoint or expired session. Fix the account issue and restart or re-import the session to check recovery.
- **NEW:** **Profile URLs as targets** - A target may now be a complete profile URL such as `https://www.instagram.com/someuser/`, in the setup wizard, on the command line and in `TARGET_USERNAMES`. It is stored as the username.
- **NEW:** **Exposure report** - `--exposure` shows identity usage, failure categories and circuit-breaker state without account names, target names or local paths. It also reports whether the ledger exists yet and whether this run could save it, how much of the daily budget is left and that it resets at local midnight, how many other accounts share the file and how many of them are stopped
...
- **IMPROVE:** **Setup and Doctor improvements** - Setup preserves saved settings, lets you skip unavailable answers and offers log, CSV and alert choices. Choosing an existing Instaloader session now checks that a session file for that account exists. It also asks whether to collect followers and following, followers only, or counts only with no names, and asks where to read them from only when it will read them.
- **IMPROVE:** **Clearer browser session import** - The profile list in setup and on the Web Dashboard marks the profiles signed in to Instagram with `*`, and defaults to the signed-in one when there is only one. The setup login menu also reports which browsers have a signed-in profile at all, before you choose one.
...
- **IMPROVE:** **Reels can be left alone, and are by default** - The endpoint Instagram reports a reel count with has become unreliable, failing regardless of the account or the IP address, and the only way left to get the count is to read the target's whole reel list. Reels are therefore no longer monitored by default. Posts and stories are unaffected, and a reel still counts towards the posts number, so a new reel moves that. Turn reels back on with **`FETCH_REELS`**, **`--fetch-reels`**, the Web Dashboard or the question `--setup` now asks, and expect repeated errors on the checks Instagram refuses
...
- **BUGFIX:** **Firefox from the Microsoft Store is found on Windows** - Session import searched only `%APPDATA%\Mozilla\Firefox\Profiles`, so a machine whose only Firefox came from the Store was told no `cookies.sqlite` was found. It now also searches the Store package's profile folder under `%LOCALAPPDATA%\Packages`, follows a redirected `APPDATA` or `LOCALAPPDATA`, and lists those profiles as **Microsoft Store** so the `default-release` both installs create can be told apart. Container imports mount a single fixed path and still cover the regular installer only (thanks [@VecToR-1024](https://github.com/VecToR-1024), from [#134](https://github.com/misiektoja/instagram_monitor/pull/134))
- **BUGFIX:** **Story viewing warnings** - Stories loaded at startup now carry the same anonymity warning as newly detected stories in both dashboards. The Web Dashboard also warns for older story records and requires confirmation before opening a story, including with a middle-click. Downloaded images offer **View Media** instead of a broken video control. **Play Video** uses downloaded videos only.
...
- **BUGFIX:** **Safer configuration and secret updates** - Configuration replacement creates a backup when a filename is passed. Shell redirection bypasses this protection. Setup preserves retained credentials when changing destinations and keeps inline secrets out of configuration backups.
- **BUGFIX:** **Safer notification delivery** - Webhook retries keep their original destination and credentials. Discord templates cannot enable mentions and invalid templates are rejected before delivery. Error messages redact credentials, including SMTP rejection replies.
