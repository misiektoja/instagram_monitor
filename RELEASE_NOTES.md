...
- **NEW:** **Offline follow relationship analysis** - Use **`--analyze-follows`** or the Web Dashboard's chart button to see mutual accounts, accounts that do not follow you back and fans, all from follower and following lists you already saved and without making a single Instagram request. The result shows when each snapshot was saved and warns when the two are at least an hour apart. Totals cover every account while the two per-account lists show up to 500 usernames each (thanks [@dogukannparlak](https://github.com/dogukannparlak), from [#121](https://github.com/misiektoja/instagram_monitor/pull/121))
- **IMPROVE:** **Errors that tell you what to do** - A failed check while monitoring now ends with the same `To fix:` action and `Guide:` link that setup and **`--doctor`** already showed. **DNS failures** name the real cause instead of reading as a generic network problem, which matters on devices that start monitoring before the network is up. Transport errors no longer trail off into **libcurl C API** links: a failed lookup now reads `Could not resolve host: www.instagram.com` and says that monitoring resumes on its own once DNS works. Add **`--debug`** when you want the underlying transport detail
- **IMPROVE:** **A Doctor you can run on a broken setup** - **`--doctor`** now reports a configuration file it cannot accept as a `FAIL` naming the line and the reason, instead of exiting before any check runs, so you can point it at a configuration you are still fixing. Every failure and warning ends with a `To fix:` action and, where one applies, a `Guide:` link taken from the same hints monitoring uses. Doctor also checks **`SENDER_EMAIL`** and **`RECEIVER_EMAIL`** before connecting, so working SMTP credentials with placeholder addresses no longer pass as `Email (SMTP) login works` and then fail on every notification. Its progress line now redraws cleanly instead of leaving text such as `Contacting Instagram ...testuser ...` behind
- **IMPROVE:** **Lighter human mode** - The BeHuman followee visit reads only the first page of accounts your session follows before picking one, instead of paginating the entire list. On an account following thousands, one simulated visit previously cost dozens of extra API requests, working against the very detection risk the feature exists to reduce
- **IMPROVE:** **Multi-target monitoring no longer queues behind a progress bar** - Targets share one terminal line, so only one follower download can draw a progress bar. A target that starts while another bar is active now fetches immediately without one, instead of waiting minutes for the other download to finish
- **IMPROVE:** **Clearer ntfy customization** - The generated configuration now states that **`WEBHOOK_TEMPLATE`**, **`WEBHOOK_USERNAME`** and **`WEBHOOK_AVATAR_URL`** apply to Discord only and are ignored by ntfy, which needs no template. Customize ntfy delivery through **`WEBHOOK_HEADERS`** such as `X-Priority` or `X-Tags`
- **IMPROVE:** **Guided issue reporting and contribution** - Opening an issue now offers a **bug report** or **feature request** form that asks for the version, install method, operating system, login method and `--doctor` output up front, while suspected vulnerabilities are routed to private reporting instead of a public issue. New [CONTRIBUTING.md](https://github.com/misiektoja/instagram_monitor/blob/main/CONTRIBUTING.md), [CODE_OF_CONDUCT.md](https://github.com/misiektoja/instagram_monitor/blob/main/CODE_OF_CONDUCT.md) and a pull request template describe the development setup, the checks CI enforces and what a change needs before it is merged
...
- **IMPROVE:** **Dependency transparency and continuous scanning** - New [THIRD_PARTY_NOTICES.md](https://github.com/misiektoja/instagram_monitor/blob/main/THIRD_PARTY_NOTICES.md) lists every runtime, optional, build and documentation dependency with its license and purpose, names the container base image and records the instaloader patches this tool carries. A supply chain workflow audits the resolved dependency tree with **`pip-audit`**, publishes a **CycloneDX SBOM** of what you actually install and scans the container image, while **CodeQL** analyzes the source and **OpenSSF Scorecard** rates the project's security practices. Everything runs on each change and again weekly, so a vulnerability published after a merge is still caught

**Bug fixes**:

- **BUGFIX:** **Trustworthy follower and following alerts** - Email and webhook alerts now ignore reported count fluctuations only after a complete list comparison confirms that no usernames changed. No-login mode, skipped or failed list fetches plus configured fetch limits keep their count-change alerts. Stopped, interrupted or intentionally limited downloads no longer become the saved comparison baseline, so the last complete list stays available. A follow and an unfollow between two checks now also logs **Followers list changed** with the unchanged count, a message that previously could never appear (closes [#118](https://github.com/misiektoja/instagram_monitor/issues/118) and [#119](https://github.com/misiektoja/instagram_monitor/issues/119))
- **BUGFIX:** **Working webhook delivery tests** - **`--send-test-webhook`** and the dashboard **Test Webhook** button now send even when the status, followers and errors switches are all off, which is the shipped default, so a correct webhook URL no longer looks broken. A **`WEBHOOK_URL`** left at its `your_webhook_url` placeholder counts as "not configured" rather than failing `--doctor` and logging a delivery error on every alert. Real event notifications still follow their configured switches and a suppressed one is explained under `--debug`
- **BUGFIX:** **Human mode without hashtags** - Leaving **`MY_HASHTAGS`** empty now skips only the hashtag request instead of aborting the whole simulation with repeated warnings and error notifications. The Explore, profile and followee actions continue to run
- **BUGFIX:** **Stable long-running dashboard sessions** - Saved settings and session changes now restart a target's monitoring context through a loop instead of re-entering it, so hundreds of live changes in one session can no longer exhaust Python's recursion limit and stop a monitor. Every active target is woken without losing the shared refresh signal, a monitor that does not stop within the timeout keeps ownership of its target so no duplicate thread starts and a restart after a flagged session keeps your **`SKIP_FOLLOW_CHANGES`** choice
- **BUGFIX:** **Validated live setting changes** - Web Dashboard updates now reject malformed types, unsafe URLs, invalid ports, out-of-range intervals and reversed hour ranges before any live value changes and a valid interval change reschedules liveness immediately. An unsupported **`CURL_CFFI_IMPERSONATE`** or **`--impersonate`** value is rejected before saving and stops the tool at startup with a message naming supported targets, instead of turning every Instagram request into a generic connection error
- **BUGFIX:** **Reliable media downloads and dashboard links** - Images and videos are streamed to bounded temporary files, checked for complete delivery and validated by media signature before atomically replacing a saved file, so truncated responses, oversized bodies and HTML error pages leave existing files untouched. Consecutive stories, posts and reels start with fresh media state so an item without a download cannot reuse the previous item's file or URL, new story-item webhooks no longer depend on email notification settings and media refreshed each check is no longer dropped from the dashboard link table once the 1000-item limit is reached
- **BUGFIX:** **Older configuration files keep loading** - A configuration file generated by version 3.0 no longer stops startup. Settings a later release removed or renamed, such as the `DISCORD_*` limits that became **`WEBHOOK_*`** in 3.1, are now **ignored with a note** telling you which lines you can delete, while a misspelled setting is still reported as an error
- **BUGFIX:** **Settings that apply from the first moment** - **`CLEAR_SCREEN = False`** in a configuration file now really prevents the startup screen clear, because the screen is no longer cleared before the configuration is read. `COLORED_OUTPUT` applies to the startup banner for the same reason and the startup internet check honors **`CHECK_INTERNET_URL`** and **`CHECK_INTERNET_TIMEOUT`** instead of the built-in defaults, so a custom probe address for a restricted network is no longer ignored
- **BUGFIX:** **Faster recovery from a flagged session** - The shared check that distinguishes a deleted target from a blocked session or IP no longer holds its lock across the network request, so other targets hitting the same failure wait for that one result rather than blocking for the length of an Instagram request and its retries. Human-like delays and long rate-limit backoff now apply only to Instagram sessions, leaving webhooks, media downloads and proxy or IP checks independent
- **BUGFIX:** **Terminal restored after quitting the dashboard** - Pressing **q** in the terminal dashboard or interrupting with Ctrl+C now restores normal typing and echo modes before the tool exits, instead of leaving the terminal in the line mode the keyboard handler uses
- **BUGFIX:** **Accurate dashboard startup errors** - A failure while starting the dashboard server is reported as a port conflict only when the address really is in use or refused. Other startup failures now show their own cause
- **BUGFIX:** **Dashboard targets survive a restart** - Targets added or removed in the browser are now saved to **`TARGET_USERNAMES`** when you press **Generate Config**, so a restarted service monitors the same list instead of starting empty. The file is written in the working directory the tool was started from and the confirmation now reports its full path, which makes it clear when a service running in `/` is not updating the file passed to **`--config-file`** (closes [#122](https://github.com/misiektoja/instagram_monitor/issues/122))

**Security and privacy**:

- **SECURITY:** **The Web Dashboard answers only you** - The dashboard now serves only requests addressed to `127.0.0.1`, `localhost`, `::1` or your configured `WEB_DASHBOARD_HOST`, so a web page that points its own domain at your machine cannot reach it through your browser. Anything that changes state must come from the dashboard page itself and send `Content-Type: application/json`, so another site can no longer stop your monitoring, force extra Instagram polling, clear your activity log or fire test notifications. Rejected requests return **HTTP 403** or **HTTP 415**. The new **`WEB_DASHBOARD_ALLOWED_HOSTS`** setting lists extra names you deliberately open the dashboard at and the single entry `"*"` restores the previous behavior. Scripts that call the API keep working when they address `127.0.0.1` and send a JSON content type
- **SECURITY:** **Saved credentials stay with their destination** - Changing **`SMTP_HOST`** or **`SMTP_PORT`** from the dashboard without retyping the password now clears the saved SMTP password instead of offering it to the new mail server, so re-enter it in the same save to keep email working. Pointing **`WEBHOOK_URL`** at a different server likewise clears **`NTFY_ACCESS_TOKEN`**, while changing only the ntfy topic on the same server keeps it
- **SECURITY:** **Your browser never contacts Instagram for the dashboard** - The **Last Fetched** panel shows a thumbnail only when the monitor already saved that image locally, so opening the dashboard no longer reveals your address to Instagram or links it to the accounts you watch. Items whose download failed show a placeholder. Post, story and thumbnail links supplied by Instagram are limited to `http` and `https` and escaped before display, so a crafted media URL cannot inject markup or a `javascript:` link
- **SECURITY:** **Narrower dashboard write surface** - Generated configuration files must be named `*.conf`, so that endpoint cannot overwrite a script or a dotfile in your working directory, the view mode accepts only its two real values and media file names built from Instagram-supplied post codes are reduced to the characters a real post code uses before they become a path. Firefox cookies are imported only from the profiles the dashboard detected and listed and only for `instagram.com` and its real subdomains rather than suffix lookalikes, media URLs serve only files the monitor registered and webhook or proxy URLs stay out of settings responses. The **CSV file name** field accepts a file name only, while an absolute `CSV_FILE` path from the config or `-b` keeps working
- **SECURITY:** **Configuration files are read, not executed** - A configuration file is now parsed as a list of settings instead of being run as Python and only `SETTING = value` lines with a recognized name and a plain value are accepted. Imports, function calls and any other code are refused, naming the offending line. A rejected file applies nothing. This matters because the first configuration searched is the one in your **current directory**, so starting the tool inside a downloaded archive or a shared directory previously ran whatever that file contained. Existing valid configurations, including generated and wizard-written ones, load unchanged
- **SECURITY:** **Instagram text cannot act on your machine** - Biographies, captions, story text, comments and usernames are stripped of terminal control sequences before anything is printed or logged, so a crafted profile can no longer clear your screen, retitle your window or overwrite a line you already read. Text starting with `=`, `+`, `-`, `@`, a tab or a carriage return is written to CSV with a leading apostrophe so opening the export cannot execute a formula, while follower and post counts stay numeric. The same text plus error messages are HTML-escaped in email alerts and showing an image through **`IMGCAT_PATH`** launches the viewer directly instead of building a shell command
- **SECURITY:** **Notification text stays out of request URLs** - ntfy titles and messages are sent as request headers or as the request body instead of query parameters, because alert text can contain follower names, captions and biographies while servers and proxies routinely log full URLs. Webhook requests go through one path that requires HTTPS, verifies certificates and refuses redirects, so a moved destination cannot collect custom **`WEBHOOK_HEADERS`** meant for the address you configured
- **SECURITY:** **Private configuration backups** - Replacing a configuration file through the setup wizard or `--generate-config` created its timestamped `.bak` copy with default permissions, leaving a world-readable backup beside a configuration you had kept owner-only. Backups are now created owner-only and keep the permissions of the file they copy
- **SECURITY:** **Safer container and release builds** - The runtime image removes **`pip`** and its build-time packages after dependencies are installed, which clears the two fixable high-severity findings and reduces what ships in production without changing how the image starts. Starting the dashboard inside a container still switches the bind address to `0.0.0.0` for Docker but now says so, shows the `-p 127.0.0.1:PORT:PORT` form that keeps it on your machine and warns what publishing it openly would expose. Every workflow pins third-party actions to a specific commit, so a moved tag cannot change what runs in the jobs that hold publishing credentials
