...
- **NEW:** Added **Webhook Notifications** system compatible with **Discord** and other webhook services for all monitored events with support for sending local files; check the [Webhook Notifications](https://github.com/misiektoja/instagram_monitor#webhook-notifications) for more info (thanks [@Sha-Dox](https://github.com/Sha-Dox) for initial implementation)
...
- **IMPROVE:** Further improvements in **error messages** for **Instagram challenge/shadow ban detection** - when Instagram requires a challenge/re-login or temporarily shadow bans the IP, error messages now provide clear, informative explanations instead of cryptic **KeyError 'data'** messages
