...
The Configuration section also reports whether [TLS verification](configuration.md#tls-verification) is on, and warns while it is off. Settings that control timing and counts, such as `INSTA_CHECK_INTERVAL`, the follower batch limits and the check-hour ranges, are checked for usable values and every one that fails is named in a single row. It resolves `LOCAL_TIMEZONE`. It reports the detected zone when the setting is `Auto` and fails when the zone is invalid or cannot be detected.
...
- **Verbose mode (`--verbose`)** also logs operational events such as follower and following counts and an alert channel switched off because its settings are still placeholders. It prints nothing per check, so an uneventful run stays quiet.
